...
const StudentSchedule = () => {
  ...
  const loadScheduleData = async () => {
    ...
    try {
      ...
      let response;
      if (viewMode === 'week') {
        response = await studentFeaturesService.getWeeklySchedule(studentId);
      } else {
        response = await studentFeaturesService.getDailySchedule(studentId, selectedDate.toISOString().split('T')[0]);
      }
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500">
          Xem lịch học hàng tuần và theo ngày của bạn
        </p>
      </div>
                  ...
                  <div key={dayIndex} className="p-2 border-r border-gray-100">
                    {classAtTime ? (
                      <div className={`p-2 rounded text-xs ${getTimeSlotColor(classAtTime.startTime)}`}>
                        ...
                        <div className="text-xs opacity-75">{classAtTime.teacherName || 'N/A'}</div>
        ...
        <div className="space-y-4">
          {getScheduleForDay(selectedDate).length > 0 ? (
              ...
              <div key={index} className="bg-white shadow rounded-lg p-6">
                ...
                <div className="mt-4 flex items-center justify-between">
                  <div className="flex items-center text-sm text-gray-600">
                    ...
                    <span>Giáo viên: {item.teacherName || 'N/A'}</span>
                  </div>
                  <div className="flex items-center text-sm text-gray-600">
            ...
            <div className="text-center py-12">
              ...
              <h3 className="mt-2 text-sm font-medium text-gray-900">Không có lịch học</h3>
              <p className="mt-1 text-sm text-gray-500">
                Không có lịch học nào vào ngày {selectedDate.toLocaleDateString('vi-VN')}
              </p>
            </div>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white shadow rounded-lg p-6">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className="text-sm font-medium text-gray-500">Giờ học/tuần</p>
              <p className="text-2xl font-semibold text-gray-900">
                {schedule.length * 2}
              </p>
            </div>
